...
public class CircularDoublyLinkedQueue<E> implements Deque<E>{
	 ...
	 * This means that we can insert, remove and inspect elements from the front and rear of the queue.
	 *  
	 ...
	 * 
	 * One implementation we will study is the one with a Circular Doubly Linked List. 
	 * Using the provided Deque interface, implement the Deque ADT using a 
	 * Circular Doubly Linked List-type structure with ONLY A DUMMY HEADER (NO DUMMY TRAILER).
	 * 
	 * Note: Throw an EmptyQueueException when Queue is empty and you try to access an element from the queue.
	 */
	...
	public void addFirst(E elm) {
		if (currentSize == 0) {
			Node<E> newNode = new Node<>(elm, header, header);
			header.setNext(newNode);
			header.setPrev(newNode);
		} else {
			Node<E> newNode = new Node<>(elm, header.getNext(), header);
	...
	public void addLast(E elm) {
		if (currentSize == 0) {
			...
			Node<E> newNode = new Node<>(elm, header, header.getPrev());
