...
public class ListHeapWrapper {
    ...
    public static class ListHeap<K, V> implements Heap<K, V> {
        ...
        private void downHeap(int i) {
            ...
            int right = 2 * i + 2;
            int min = i;
            if (left < elements.size() && comparator.compare(elements.get(left).getKey(), elements.get(min).getKey()) < 0)
                min = left;
            if (right < elements.size() && comparator.compare(elements.get(right).getKey(), elements.get(min).getKey()) < 0)
                min = right;
            if (min != i) {
                Entry<K, V> temp = elements.get(i);
