...
public class HashTableOA<K, V> implements Map<K, V> {
	...
	public HashTableOA() {
		/**
		 * Simple Hash Function Implementation Lambda passed to constructor 
		 * (See SimpleHashFunction.java for class definition reference)
		 */
	...
	public void rehash() {
		...
		Bucket<K, V>[] newBuckets = new Bucket[newSize];
		...
		for (Bucket<K, V> bucket : buckets) {
			...
			int i = targetBucket;
			int j = -1;
			while (newBuckets[i].isInUse()) {
				if (newBuckets[i].getKey().equals(bucket.getKey())) {
					newBuckets[i].setValue(bucket.getValue());
					j = i;
					break;
				}
				i = (i + 1) % newSize;
