...
public class TicketProblem {
	 ...
	 */
	public int satisfiedCustomers(int[] ticketNeeds, int available) {
		...
		while (!line.isEmpty() && available > 0) {
			int personInLine = line.dequeue();
			if (ticketNeeds[personInLine] > 0) {
				ticketNeeds[personInLine]--;
				available--;
				if (ticketNeeds[personInLine] == 0) {
					satisfiedCustomers++;
				}
				line.enqueue(personInLine);
			}
		}
