...
public class BinaryNumberSequenceProblem {
	 ...
	 * 
	 * You must store the results of the method in the queue provided as a private field.
	 * 
	 * Note: Use one of the Queue Implementations you created in a previous exercise
	 * 
	 * WARNING: You MUST create and store the binary number using division, mods, a queue, or something similar. 
	 * Solutions that use Integer.toBinaryString(number) to avoid using doing so will receive ZERO credit.
	 */

	public void binaryNumberSequence(int N) {
		Queue<String> sequenceQueue = new LinkedQueue<>();
		for (int i = 1; i <= N; i++) {
			int decimalNumber = i;
			...
			sequenceQueue.enqueue(binaryNumber.toString());
		}
		this.binarySequence = sequenceQueue;
	}
}