    ...
    public static int minimumMoves(List<Integer> board) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < levelSize; i++) {
                ...
                for (int j = 1; j <= 6; j++) {
                    int nextPos = current.getX() + j;

                    if (nextPos < size && !visited[nextPos]) {
                        int boardValue = board.get(nextPos);
                        int nextX = (boardValue == -1) ? nextPos : boardValue;
                        Coordinate nextCoord = new Coordinate(nextX, current.getY() + 1);
